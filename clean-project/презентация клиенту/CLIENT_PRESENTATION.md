...
1. Включите отчёты заказов в каждом партнёрском кабинете (Flipkart Affiliate → Reports, Amazon Associates → Earnings, Ajio/Croma/Nykaa — через менеджера).
2. При генерации ссылок передавайте Telegram user ID (или username) в суб-ID/Custom Tag, чтобы знать, кому начислять вознаграждение.
3. Собирайте отчёты в таблицу `cashback_transactions` и отдавайте суммы в `RealDataService` — бот покажет их в карточках и уведомлениях.
