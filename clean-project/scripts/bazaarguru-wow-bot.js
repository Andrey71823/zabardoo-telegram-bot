...
// AI Services для распознавания изображений и голоса
...
class BazaarGuruAggregatorBot {
  ...
  setupCommands() {
    const commands = [
      ...
      { command: 'profile', description: 'Profile — coming soon' },
      { command: 'cashback', description: 'Cashback — coming soon' },
      { command: 'language', description: 'Switch language' },
      { command: 'help', description: 'FAQ and contacts' }
    ];

    Promise.resolve(this.bot.setMyCommands(commands)).catch((error) => {
  ...
  async sendProfileComingSoon(chatId, user) {
    const body = this.t(user, 'messages.profileSoon');
  ...
  async sendCashbackGuide(chatId, user) {
    const body = this.t(user, 'messages.cashbackSoon');
    await this.bot.sendMessage(chatId, body, {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [
  ...
  async handleMessage(msg) {
    ...
    if (lower === '/deal' || lower === '/deals' || lower === 'deal' || lower === 'deals') {
      const deals = rankProducts(this.products);
      return this.sendDealsFromList(chatId, user, this.t(user, 'deals.hotTitle'), deals, {
        contextId: 'command:deals'
      });
    }

    if (lower === '/profile' || lower === 'profile') {
      return this.sendProfileComingSoon(chatId, user);
    }

    if (lower === '/cashback' || lower === 'cashback') {
      return this.sendCashbackGuide(chatId, user);
    }
    ...
    if (msg.voice) {
      return this.handleVoiceMessage(chatId, user, msg);
    }

    // Обработка фото для поиска товаров
    if (msg.photo) {
      return this.handlePhotoMessage(chatId, user, msg);
    }
  ...
  /**
   * Обработка голосовых сообщений - распознавание речи и поиск
   */
  async handleVoiceMessage(chatId, user, msg) {
    try {
      // Показываем что обрабатываем
      await this.bot.sendMessage(chatId, '🎤 ' + this.t(user, 'search.processingVoice'), { parse_mode: 'HTML' });
      ...
      // Распознаём голос через AI
      const result = await this.voiceService.transcribeFromUrl(fileUrl);

      if (result.error || !result.text) {
        return this.bot.sendMessage(
          chatId,
          '❌ ' + this.t(user, 'search.voiceError'),
          { parse_mode: 'HTML' }
        );
      }
      ...
      await this.bot.sendMessage(
        chatId,
        `🔍 ${this.t(user, 'search.voiceRecognized')}: "<b>${result.text}</b>"`,
        { parse_mode: 'HTML' }
      );

      // Выполняем поиск по распознанному тексту
      return this.processSearch(chatId, user, result.text);
  ...
  /**
   * Обработка фото - распознавание товара и поиск похожих
   * Поддерживает фото + текст (подпись к фото)
   */
  async handlePhotoMessage(chatId, user, msg) {
    try {
      // Получаем подпись к фото если есть
      const caption = msg.caption ? msg.caption.trim() : null;
      
      // Показываем что обрабатываем
      const processingMsg = caption 
        ? `📸 ${this.t(user, 'search.processingImageWithText')}: "${caption}"`
        : '📸 ' + this.t(user, 'search.processingImage');
      await this.bot.sendMessage(chatId, processingMsg, { parse_mode: 'HTML' });
      ...
      // Анализируем изображение через Gemini (с учётом подписи)
      const analysis = await this.geminiService.analyzeImageFromUrl(fileUrl, caption);

      if (analysis.error || !analysis.keywords?.length) {
        return this.bot.sendMessage(
          chatId,
          '❌ ' + this.t(user, 'search.imageError'),
          { parse_mode: 'HTML' }
        );
      }

      // Формируем сообщение о распознанном
      const recognizedInfo = [
        `📷 <b>${this.t(user, 'search.imageRecognized')}</b>`,
        '',
        `📝 ${analysis.description}`,
        analysis.brand ? `🏷 Brand: ${analysis.brand}` : '',
        `🏪 ${this.t(user, 'search.category')}: ${analysis.category}`,
        '',
        `🔍 ${this.t(user, 'search.searchingFor')}: <b>${analysis.suggestedSearch || analysis.keywords.join(', ')}</b>`
      ].filter(Boolean).join('\n');

      await this.bot.sendMessage(chatId, recognizedInfo, { parse_mode: 'HTML' });

      // Выполняем поиск по ключевым словам
      const searchQuery = analysis.suggestedSearch || analysis.keywords.join(' ');
      return this.processSearch(chatId, user, searchQuery);
  ...
  async processSearch(chatId, user, rawQuery) {
    ...
    if (!total) {
      // AI анализ неудачного поиска
      const analysis = await this.geminiService.analyzeFailedSearch(query, user.language);
      
      let message = this.t(user, 'searchNone');
      
      if (analysis) {
        const lines = [
          `🔍 <b>${this.t(user, 'search.analyzed')}</b>`,
          '',
          `📝 ${analysis.understood}`,
          `🏷 ${this.t(user, 'search.category')}: ${analysis.category}`,
        ];
        ...
        if (analysis.suggestions && analysis.suggestions.length) {
          lines.push('');
          lines.push(`💡 <b>${this.t(user, 'search.suggestions')}:</b>`);
          analysis.suggestions.forEach(s => lines.push(`  • ${s}`));
        }
        
        if (analysis.exampleQueries && analysis.exampleQueries.length) {
          lines.push('');
          lines.push(`🔎 <b>${this.t(user, 'search.trySearching')}:</b>`);
          analysis.exampleQueries.forEach(q => lines.push(`  • "${q}"`));
        }
  ...
  async sendDealsFromList(chatId, user, title, deals, options = {}) {
    ...
    const pageSize = options.pageSize || 3; // По 3 товара на страницу
    ...
    if (contextId && filtered.length > pageSize) {
      const paginationRow = [];
      ...
      if (start > 0) {
        paginationRow.push({
          text: '◀️',
          callback_data: `list_more|${contextId}|${Math.max(0, start - pageSize)}`
        });
      }
